#!/usr/bin/env python
"""
This script generates a number of random high-level stimulus, into files. An alphabet is needed for the high-level stimulus part. Ranges are needed for variables that are not boolean. 
...
def main(n,s_vs_c):
	for i in range (0,int(n)): #Number of tests required
		#Open files to write
		f = open('stimulus_'+str(i+1)+'.txt', 'w')
		x = random.randint(0, 100000) #Specify amount of seeds available
		print 'Seed '+ str(i+1)+':'+ str(x)
		random.seed(x)
		k = random.randint(4, 15) #Specify length of random traces
