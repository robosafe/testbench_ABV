...
sleep 15
echo "** Simulator launched (bert2_gazebo_motor_controller respawns)**"
...
while [ $COUNTER -lt $NTESTS ]; do
  ...
  (rosrun bert2_gazebo human_2.py stim_smoothRun >> /tmp/humanoutr$COUNTER & echo $! >> /tmp/rospids) &
  ...
  sleep 10
  echo "Reawoken.  Checking code coverage."
